	...
	std::cout << "ERASE" << std::endl;
{
	...
	for (int i = 0; i < 10; i++) {
		t1_std = std::clock();
		str_std0.erase(str_std0.begin() + (n/4), str_std0.begin() + (n/2));
		t1_std = std::clock() - t1_std;
		t5_std+= t1_std;
		str_std0.insert(str_std0.begin(), n/4, "a");
	}

	for (int i = 0; i < 10; i++) {
		t2_ft = std::clock();
		str_ft0.erase(str_ft0.begin() + (n/4), str_ft0.begin() + (n/3));
		t2_ft = std::clock() - t2_ft;
		t6_ft += t2_ft;
		str_ft0.insert(str_ft0.begin(), n/4, "a");
	}

	for (int i = 0; i < 10; i++) {
		t3_std = std::clock();
		int_std0.erase(int_std0.begin() + (n/4), int_std0.begin() + (n/3));
		t3_std = std::clock() - t3_std;
		t7_std += t3_std;
		int_std0.insert(int_std0.begin(), n/4, 2);
	}

	for (int i = 0; i < 10; i++) {
		t4_ft = std::clock();
		int_ft0.erase(int_ft0.begin() + (n/4), int_ft0.begin() + (n/3));
		t4_ft = std::clock() - t4_ft;
		t8_ft += t4_ft;
		int_ft0.insert(int_ft0.begin(), n/4, 2);
	}
